...
def check_and_fix_inf_nan(input_tensor, loss_name="default", hard_max=MAX_DISP):
    """
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    # L1 distance
    # loss_reg = torch.abs(gt[mask] - pred[mask])
...
def gradient_loss(prediction, target, mask, conf=None, gamma=1.0, alpha=0.2):
    ...
    grad_x = grad_x.clamp(max=MAX_DISP)
    grad_y = grad_y.clamp(max=MAX_DISP)
