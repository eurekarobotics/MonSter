...
@hydra.main(version_base=None, config_path="config", config_name="train_mixed_small_muon_dinov3s")
def main(cfg):
    set_seed(cfg.seed)
    ...
    # Initial evaluation before training starts (important when resuming from checkpoint)
    if cfg.restore_ckpt is not None:
        ...
        best_epe = init_epe  # Set initial best_epe to loaded model's performance
        print(f"Initial checkpoint EPE: {init_epe:.4f}, D1: {init_d1:.2f}%")
        accelerator.log(
            {
                "val/epe": init_epe.item(),
                "val/d1": init_d1.item(),
            },
            total_step,
        )
